...
import ru.gmail.gasimov.task3.comparator.FirstPointXComparator;
...
public class MainForSort {
    ...
    public static void main(String[] args) throws TriangleException {
        ...
        TriangleTypeComparator triangleTypeComparator = new TriangleTypeComparator();
        FirstPointXComparator firstPointXComparator = new FirstPointXComparator();

        List<Triangle> sortedByTriangleType = repository.sort(triangleTypeComparator);
        List<Triangle> sortedByFirstX = repository.sort(firstPointXComparator);

        for (Triangle triangle : sortedByTriangleType) {
            LOGGER.log(Level.INFO, triangle);
        }

        for (Triangle triangle : sortedByFirstX) {
            LOGGER.log(Level.INFO, triangle);
        }
    }
}
